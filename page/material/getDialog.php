<?php
	...
	if($_GET['table'] == 'material.product'){
		if(isset($_GET['Dcol']) && $_GET['Dtext'] != ''){
			...
			$SQL = "SELECT {$_GET['cols']} FROM $table 
				WHERE compcode='$compcode' AND active='1'".$addSql.
				"ORDER BY sysno DESC";
				
		}else{
			$SQL = "SELECT {$_GET['cols']} FROM $table  WHERE compcode='$compcode' AND active='1' ORDER BY sysno DESC";
		}
	}
	else {
		if(isset($_GET['Dcol']) && $_GET['Dtext'] != ''){
			...
			$SQL = "SELECT {$_GET['cols']} FROM $table 
				WHERE compcode='$compcode' AND recstatus='A'".$addSql.
				"ORDER BY sysno DESC";
				
		}else{
			$SQL = "SELECT {$_GET['cols']} FROM $table  WHERE compcode='$compcode' AND recstatus='A' ORDER BY sysno DESC";		}
	}
