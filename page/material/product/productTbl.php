<?php
	...
	if(isset($_GET['Stext']) && $_GET['Scol'] == 'itemcode'){
		$SQL = "SELECT sysno, compcode, itemcode, description, uomcode, groupcode, productcat, suppcode,
				CASE WHEN active = 1 THEN 'Yes' ELSE 'No' END AS active, 
				avgcost, actavgcost, currprice, qtyonhand, bonqty,
				CASE WHEN rpkitem = 1 THEN 'Yes' ELSE 'No' END AS rpkitem, 
				minqty, maxqty, reordlevel, reordqty, adduser, adddate, upduser, upddate, 
				CASE WHEN chgflag = 1 THEN 'Yes' ELSE 'No' END AS chgflag, 
				subcatcode, 
				CASE WHEN expdtflg = 1 THEN 'Yes' ELSE 'No' END AS expdtflg, 
				mstore, costmargin, pouom, 
				CASE WHEN reuse = 1 THEN 'Yes' ELSE 'No' END AS reuse, 
				trqty, deactivedate, 
				CASE WHEN tagging = 1 THEN 'Yes' ELSE 'No' END AS tagging, 
				itemtype, generic
				FROM $table 
				WHERE compcode='$compcode' AND 
				{$_GET['Scol']} LIKE '{$_GET['Stext']}%' 
