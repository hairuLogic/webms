		...
		$(document).ready(function () {
			...
			function checkIfTillOpen(){
				var param={
					...
					field:['tillcode','opendate','opentime','closedate','closetime','cashier'],
					except:'',
					table_name:'debtor.tilldetl',
					filter:{cashier:'session.username',closedate:'IS NULL'}
				}
			...
			$( "#tilldet" ).dialog({
				...
				open: function() { 
					checkIfTillOpen();
					dialogHandler('debtor.till','tilldetTillcode',['tillcode','description','tillstatus'], 'Select Till');
			...
			function emptyFormdata(form){
				if(form == '#formdata')errorField.length=0;
				$(form).trigger('reset');
			...
			$("#gridDialog").jqGrid({
				...
				colModel: [
					...
					{ label: 'tillstatus', name: 'tillstatus', width: 200, classes: 'pointer', canSearch:true},
				],
				width: 680,
				viewrecords: true,
				loadonce: false,
